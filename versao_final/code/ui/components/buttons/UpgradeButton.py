...
class UpgradeButton(Button):
    def __init__(self, x, y, on_click_args, on_click):
        ...
        super().__init__(x, y, self.__settings.UPGRADE_BUTTON_WIDTH, self.__settings.UPGRADE_BUTTON_HEIGHT, 
                         self.__settings.UI_BG_COLOR, self.__settings.UI_HOVER_COLOR, on_click_args=on_click_args,
                         on_click=on_click)
    ...
    def draw(self):
        if self.enabled:
            ...
            # conteúdo
            # nome
            name_surf = self.font.render(self.on_click_args.name, False, self.__settings.TEXT_COLOR)
            ...
            pygame.draw.rect(self.display_surface, self.__settings.UI_BORDER_COLOR_ACTIVE, icon_rect, 4)
            ...
            while len(words) > 0:
                ...
                while True:
                    ...
                    line_surf = self.font.render(line_text, False, self.__settings.TEXT_COLOR)
            ...
            disabled_background.fill(self.__settings.UI_BORDER_COLOR)
            self.display_surface.blit(disabled_background, self.rect.topleft)
            pygame.draw.rect(self.display_surface, self.__settings.UI_BORDER_COLOR, self.rect, 4)

            # texto de indisponível
            msg_surf = self.font.render('VOCÊ NÃO POSSUI PONTOS DE UPGRADE!', False, self.__settings.TEXT_COLOR)
