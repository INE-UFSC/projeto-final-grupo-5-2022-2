...
class Tile(pygame.sprite.Sprite):
    def __init__(self, pos, tile_number=''):
        ...
        self.__image = Resources().get_sprite(f'/tiles/{self.__tile_number}.png') if len(
            self.__tile_number) > 0 else pygame.Surface((self.__settings.ILESIZE, self.__settings.TILESIZE))
        self.__rect = self.image.get_rect(topleft=pos)
        self.__hitbox = self.rect.inflate(self.__settings.X_OFFSET.get(self.__tile_number, -16),
                                          self.__settings.Y_OFFSET.get(self.__tile_number, -16))
        self.__hitbox.bottom = self.__rect.bottom
        small_hitbox_offset = self.__settings.SMALL_HITBOX_OFFSET.get(self.__tile_number, 0)
